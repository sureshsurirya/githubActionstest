...
DECLARE
  ...
  SHARE_NAME STRING;
BEGIN
  SQL_STMT := 'CREATE MANAGED ACCOUNT ' || &{CUST_NAME} || '_reader' || '
  ...
  SQL_STMT := 'CREATE OR REPLACE SHARE ' || &{CUST_NAME} || '_share';
  EXECUTE IMMEDIATE SQL_STMT;

  SQL_STMT := 'GRANT USAGE ON DATABASE &{CUST_RAW_DB} TO SHARE ' || &{CUST_NAME} || '_share';
  EXECUTE IMMEDIATE SQL_STMT;

  SQL_STMT := 'GRANT USAGE ON SCHEMA &{CUST_RAW_DB}.&{CUST_SCHEMA} TO SHARE ' || &{CUST_NAME} || '_shar'e;
  EXECUTE IMMEDIATE SQL_STMT;

  SQL_STMT := 'GRANT SELECT ON ALL TABLES IN SCHEMA &{CUST_RAW_DB}.&{CUST_SCHEMA} TO SHARE ' || &{CUST_NAME} || '_share';
  EXECUTE IMMEDIATE SQL_STMT;

  SQL_STMT := 'ALTER SHARE ' || &{CUST_NAME} || '_share' || ' ADD ACCOUNT = ''' || LOCATOR || '''';
  EXECUTE IMMEDIATE SQL_STMT;
